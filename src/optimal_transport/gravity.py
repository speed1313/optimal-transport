...
def Sinkhorn(a, b, x, y, eps):
    ...
    for i in range(n):
        for j in range(m):
            ...
            # cost as  gravity
            C = C.at[i,j].set((1/jnp.abs(x[i] - y[j]).sum()) + jnp.linalg.norm(x[i] - y[j]))
    K = jnp.exp(- C / eps)
    u = jnp.ones(n)
    v = jnp.ones(m)
    for i in range(100):
...
x = jax.random.uniform(jax.random.PRNGKey(0), (n, 2)) * 2
y = jax.random.uniform(jax.random.PRNGKey(1), (m, 2)) * 2
original_x = x.copy()
a = jnp.ones(n) / n
b = jnp.ones(m) / m
eps = 0.15
...
anim.save("gravity.gif", writer="pillow")
plt.show()
